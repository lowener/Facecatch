...
/* haar_features
 * Parameters: integral image, image, position x and y of the 24x24 area
...
{
  feature *array_feat = calloc(220000,sizeof(feature));
...
/* for (i=0; i < 24; i++) { */
  /*   for (j=0; j < 24; j++) { */
  /*     for (h=0; i+h-1<24; h++) { */
  /*       for (w=0; j+2*w-1<24; w++) { */
  /*         Uint32 S1 = integral_region(int_img, img->w, */
  /*                                     x+i, y+j,   x+i+h-1, y+j+w-1); */
  /*         Uint32 S2 = integral_region(int_img, img->w, */
...
int haar_f1 (Uint32 *int_img, size_t width, feature *array_feat, feature *current,
             int index_array, int x, int y)
{
  int i,j,h,w;
  for (i=1; i <= 24; i++) {
    for (j=1; j <= 24; j++) {
...
int haar_f2 (Uint32 *int_img, size_t width, feature *array_feat, feature *current,
             int index_array, int x, int y)
{
  int i,j,h,w;
  for (i=1; i <= 24; i++) {
...
int haar_f3 (Uint32 *int_img, size_t width, feature *array_feat, feature *current,
             int index_array, int x, int y)
{
  int i,j,h,w;
  for (i=1; i <= 24; i++) {
...
int haar_f4 (Uint32 *int_img, size_t width, feature *array_feat, feature *current,
             int index_array, int x, int y)
{
  int i,j,h,w;
  for (i=1; i <= 24; i++) {
...
int haar_f5 (Uint32 *int_img, size_t width, feature *array_feat, feature *current,
             int index_array, int x, int y)
{
  int i,j,h,w;
  for (i=1; i <= 24; i++) {
